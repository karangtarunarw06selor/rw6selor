...
(function() {
  ...
  badge.innerHTML = `<span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background-color: #4CAF50; margin-right: 6px; box-shadow: 0 0 0 0 rgba(76, 175, 80, 0.7); animation: dotPulseUmumRW06 1.5s infinite cubic-bezier(0.66, 0, 0, 1);"></span><span id="umumUserCount">0</span> Pengunjung Online`;
  
  // Tambahkan style animasi pulse
  let style = document.createElement('style');
  style.innerHTML = `@keyframes dotPulseUmumRW06 { to { box-shadow: 0 0 0 8px rgba(76, 175, 80, 0); } }`;
  ...
  function kirimPingAktifUmum() {
    fetch(`${URL_API_RW06}?aksi=pingUmum&namaUser=${encodeURIComponent(namaPengunjungUmum)}`, { cache: "no-store" })
      .then(r => r.json())
      .then(res => {
