...
/**
 * Find existing member by full_name AND birth_date (strict matching).
 * Email is optional additional check.
...
function find_existing_member(PDO $pdo, string $full_name, string $email, string $birth_date = ''): ?int
{
    ...
    if ($email !== '') {
        // Cocokkan nama + tanggal lahir + email (strict 3 field)
    ...
    $sql = "SELECT id FROM members WHERE " . implode(' AND ', $conditions) . " LIMIT 1";
