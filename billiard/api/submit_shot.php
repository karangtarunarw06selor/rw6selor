...
function pc_get_room_players_ordered($pdo, $roomId) {
    $stmt = $pdo->prepare("
        SELECT 
            player_id,
            turn_order
...
function pc_get_8ball_team_player_ids($pdo, $roomId, $playerId) {
    $players = pc_get_room_players_ordered($pdo, $roomId);
    $count = count($players);
    ...
    // index 0 + 2 = team A
    // index 1 + 3 = team B
    $teamIndexes = ($index % 2 === 0) ? [0, 2] : [1, 3];

    $teamIds = [];

    foreach ($teamIndexes as $teamIndex) {
        if (isset($players[$teamIndex])) {
            $teamIds[] = (int)$players[$teamIndex]["player_id"];
        }
    }
...
function pc_get_8ball_opponent_player_ids($pdo, $roomId, $playerId) {
    $players = pc_get_room_players_ordered($pdo, $roomId);
    $count = count($players);
    ...
    $teamIds = pc_get_8ball_team_player_ids($pdo, $roomId, $playerId);
    $opponentIds = [];
...
function pc_get_player_8ball_group($rules, $playerId) {
    $groups = $rules["player_groups"] ?? [];
    return $groups[(string)$playerId] ?? null;
}
...
function pc_is_rotation_mode($mode) {
    return in_array($mode, ["9ball", "10ball"], true);
}

function pc_get_rotation_final_ball($mode) {
    if ($mode === "9ball") {
        return 9;
    }
...
function pc_get_lowest_unpocketed_rotation_ball($balls, $finalNumber) {
    if (!$finalNumber) {
        return null;
    }
...
function pc_restore_ball_from_old_state(&$cleanNewBalls, $oldBalls, $number) {
    $oldBall = pc_find_ball_by_number($oldBalls, $number);

    foreach ($cleanNewBalls as &$ball) {
        if ((int)$ball["number"] === (int)$number) {
            $ball["pocketed"] = false;
            $ball["vx"] = 0;
            $ball["vy"] = 0;

            if ($oldBall) {
                $ball["x"] = $oldBall["x"] ?? $ball["x"];
                $ball["y"] = $oldBall["y"] ?? $ball["y"];
            }
...
$rotationTargetNumber = null;
$rotationFinalBallNumber = null;

  /*
  9 Ball / 10 Ball rotation rules.
  - Cue ball wajib first hit bola angka terendah yang masih di meja.
  - Final ball masuk legal = menang.
  - Final ball masuk saat foul = foul + final ball dikembalikan.
*/
if (pc_is_rotation_mode($matchMode)) {
    $rotationFinalBallNumber = pc_get_rotation_final_ball($matchMode);
    $rotationTargetNumber = pc_get_lowest_unpocketed_rotation_ball($oldBalls, $rotationFinalBallNumber);

    $finalBallPocketed = $rotationFinalBallNumber
        ? in_array($rotationFinalBallNumber, $newPocketedObjectBalls, true)
        : false;

    if (!$hasFoul && $rotationTargetNumber !== null) {
        if ($firstHitBallNumber === null) {
            $hasFoul = true;
            $foulReason = "rotation_no_first_hit";
        } elseif ((int)$firstHitBallNumber !== (int)$rotationTargetNumber) {
            $hasFoul = true;
            $foulReason = "wrong_first_hit_rotation";
        }
    }

    if ($finalBallPocketed) {
        if (!$hasFoul) {
            $gameFinished = true;
            $winnerPlayerId = $playerId;
        } else {
            pc_restore_ball_from_old_state($cleanNewBalls, $oldBalls, $rotationFinalBallNumber);

            if (!$foulReason) {
                $foulReason = "final_ball_illegal";
            }
        }
    }
...
*/
} elseif ($matchMode === "8ball_classic" || $matchMode === "8ball_party") {
...
$teamIds = pc_get_8ball_team_player_ids($pdo, $roomId, $playerId);
$opponentIds = pc_get_8ball_opponent_player_ids($pdo, $roomId, $playerId);

foreach ($teamIds as $teamPlayerId) {
    $eightBallRules["player_groups"][(string)$teamPlayerId] = $assignedGroupThisShot;
}

if ($opposite) {
    foreach ($opponentIds as $opponentPlayerId) {
        $eightBallRules["player_groups"][(string)$opponentPlayerId] = $opposite;
    }
}

$playerGroup = pc_get_player_8ball_group($eightBallRules, $playerId);
                }
            }
    ...
    $oldState["last_shot"] = [
        ...
        "rotation_target_number" => $rotationTargetNumber,
        "rotation_final_ball_number" => $rotationFinalBallNumber,
        "rail_foul" => $railFoul,
