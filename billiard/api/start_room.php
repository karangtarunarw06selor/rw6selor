...
function generateInitialBalls($mode) {
    ...
    $makeBall = function($number, $x, $y) use ($colors) {
        $type = "solid";
    ...
    if ($mode === "9ball") {
        // 9 Ball diamond: 1 di apex, 9 di tengah.
        $rackPositions = [
            ["x" => $startX,             "y" => $startY],
            ["x" => $startX + $gapX,     "y" => $startY - ($gapY / 2)],
            ["x" => $startX + $gapX,     "y" => $startY + ($gapY / 2)],
            ["x" => $startX + 2*$gapX,   "y" => $startY - $gapY],
            ["x" => $startX + 2*$gapX,   "y" => $startY],
            ["x" => $startX + 2*$gapX,   "y" => $startY + $gapY],
            ["x" => $startX + 3*$gapX,   "y" => $startY - ($gapY / 2)],
            ["x" => $startX + 3*$gapX,   "y" => $startY + ($gapY / 2)],
            ["x" => $startX + 4*$gapX,   "y" => $startY],
        ];

        $rackNumbers = [1, 2, 3, 4, 9, 5, 6, 7, 8];

        foreach ($rackNumbers as $index => $number) {
            $pos = $rackPositions[$index];
            $balls[] = $makeBall($number, $pos["x"], $pos["y"]);
        }
    ...
    if ($mode === "10ball") {
        // 10 Ball triangle: 1 di apex, 10 di tengah.
        $rackPositions = [
            ["x" => $startX,             "y" => $startY],
            ["x" => $startX + $gapX,     "y" => $startY - ($gapY / 2)],
            ["x" => $startX + $gapX,     "y" => $startY + ($gapY / 2)],
            ...
            ["x" => $startX + 2*$gapX,   "y" => $startY],
            ["x" => $startX + 2*$gapX,   "y" => $startY + $gapY],
            ["x" => $startX + 3*$gapX,   "y" => $startY - 1.5*$gapY],
            ["x" => $startX + 3*$gapX,   "y" => $startY - 0.5*$gapY],
            ["x" => $startX + 3*$gapX,   "y" => $startY + 0.5*$gapY],
            ["x" => $startX + 3*$gapX,   "y" => $startY + 1.5*$gapY],
        ];

        $rackNumbers = [1, 2, 3, 4, 10, 5, 6, 7, 8, 9];
    ...
    // Straight Pool / 8 Ball: triangle 1–15.
    $maxBall = 15;
    $rows = 5;
    $number = 1;
